/* ═══════════════════════════════════════════
   ...
   Model, state, presets, sidebar builder
═══════════════════════════════════════════ */
...
const GLOBAL_IMP = {
  'concave_pts_worst':0.088,'concavity_worst':0.079,'area_worst':0.065,
  'concavity_mean':0.058,'radius_worst':0.052,'palpable_lump':0.044,
  'area_mean':0.038,'radius_mean':0.031,
};
...
function scorePatient(p) {
  ...
  let tier = hasImm||prob>=0.72 ? 'URGENT' : hasPrm||prob>=0.48 ? 'HIGH' : prob>=0.28 ? 'ELEVATED' : 'ROUTINE';
  const d = Math.abs(prob-0.5);
  const confidence = (d>0.30||hasImm)?'HIGH':d>0.12?'MODERATE':'LOW';
  return { tier, prob, confidence, flags };
}
